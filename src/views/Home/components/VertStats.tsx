...
import { useBurnedBalance, useTotalSupplyPills } from 'hooks/useTokenBalance'
...
const VertStats = () => {
  ...
  const totalSupply = useTotalSupplyPills()
  /* const totalSupplyMorph = useTotalSupply() */
  const tokenAddress = getCakeAddress()
  const pillsAddress = getPillsAddress()
  const burnedBalance = useBurnedBalance(tokenAddress)
  const burnedBalancePills = useBurnedBalance(pillsAddress)
  // const farms = useFarms();
  /* const eggPrice = usePriceCakeBusd(); */
  const pillsPrice = usePricePillsMim();
  const circSupply = totalSupply ? totalSupply.minus(burnedBalancePills) : new BigNumber(0);
  /* const cakeSupply = getBalanceNumber(circSupply); */
  const marketCap = pillsPrice.times(circSupply);
    ...
    <StyledVertStats isMobile={isMobile}>
      <ColumnLayout isMobile={isMobile}>
        ...
        <Column>
          ...
          <Row>
            <Text fontSize="14px">{t('PILLS Market Cap')}</Text>
            <CardValue fontSize="14px" value={getBalanceNumber(marketCap)} decimals={0} prefix="$" />
          </Row>
          <Row>
            <Text fontSize="14px">{t('Total PILLS Minted')}</Text>
            ...
            <LinkedText fontSize="14px">{t('Total MORPH Burned')}{"   "}
